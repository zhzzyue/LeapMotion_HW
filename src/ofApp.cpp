...
void ofApp::setup(){
    ...
    water.load("47487117-landscape-pics.jpg");
    birdImg.load("birds_PNG68.png");
    birds.load("Amazing Natural Bird Sounds.mp3");
    machine.load("cut trees 400 years old  tree cutting machine.mp3");
    machineImg.load("electric-chain-saw-250x250.png");
...
//    ofDrawCircle(ofGetWindowHeight()/2-50, ofGetWindowWidth()/2,30);
...
//            if ( tip.z < -50  && tip.x >= (ofGetWindowHeight()/2-35) &&  tip.x <= (ofGetWindowHeight()/2-65) && tip.y >= (ofGetWindowWidth()/2-15) && tip.y <= ( ofGetWindowWidth()/2+15)){
// 
//                ofSetColor(255);
//                water.draw(-ofGetWindowWidth()/2,-ofGetWindowHeight()/2,50);
    ...
    birdImg.draw(ofGetWindowWidth()/4,0);
    machineImg.draw(400,600);
}
